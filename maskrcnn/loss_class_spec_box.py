...
def loss_class_spec_box_reg_fn(
        proposals, 
        valid_mask, 
        bbox_deltas_pred, 
        gt_labels,
    ...
    def loss_per_image(
        ...
        bbox_deltas_pred_valid = tf.boolean_mask(
            bbox_deltas_pred, 
            valid_mask
        )
        gt_bboxes_tensor = gt_bboxes.to_tensor()
        
        # Find the best matching ground truth for each proposal
        ious = compute_iou(proposals_valid, gt_bboxes_tensor)
        ...
        bbox_deltas_pred_pos_reshaped = tf.reshape(
            bbox_deltas_pred_pos, [-1, num_classes, 4])  # [M, C, 4]
